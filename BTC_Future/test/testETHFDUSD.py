...
MODEL_FILE = os.path.join(MODEL_DIR, "v3_model_20260213_152911.txt")       # ← เปลี่ยนเป็นชื่อ model ที่เทรนจาก Spot data (33 features)
META_FILE  = os.path.join(MODEL_DIR, "v3_model_20260213_152911_meta.json")  # ← meta ตัวเดียวกัน (33 features)
# --- TELEGRAM ---
...
try:
    ...
    # ดึง LOT_SIZE และ PRICE_FILTER จาก exchange info
    symbol_info = client.get_symbol_info(SYMBOL_TRADE)
    QTY_STEP = 0.0001   # default
...
def sync_existing_positions():
    ...
    try:
        ...
        print(f"\n📊 SYNC: {BASE_ASSET}={btc_total:.6f} | Open Orders={len(open_orders)}")
        ...
        if btc_total > min_btc:
            ...
            print(f"✅ ไม่มี ${BASE_ASSET} เดิม - พร้อมเริ่ม")
            return False
    except Exception as e:
        ...
        if order.get('order_id'):
            try:
                ...
                if status == 'FILLED':
                    ...
                    sell_qty = float(int(filled_qty * 0.999 / QTY_STEP) * QTY_STEP)
...
def check_active_orders(current_price, current_ts):
    ...
    for order in active_orders[:]:
        ...
        if is_exit:
            if "TP" in reason:
                # TP: เช็คว่า sell order filled จริงไหม ถ้ายังไม่ filled ให้รอ
                if order.get('sell_order_id'):
                    try:
                        ...
                        if status == 'FILLED':
                            pass  # filled แล้ว ดำเนินการต่อ
                        elif status == 'NEW' or status == 'PARTIALLY_FILLED':
                            continue  # ยังไม่ filled → รอก่อน ไม่ยกเลิก
                        else:
                            # CANCELED/EXPIRED → ขาย market แทน
                            close_position_spot(SYMBOL_TRADE, order['quantity'], reason)
                    except:
                        continue  # เช็คไม่ได้ → รอรอบหน้า
                else:
                    # ไม่มี sell order → ขาย market
                    close_position_spot(SYMBOL_TRADE, order['quantity'], reason)
            else:
                # SL/TIME EXIT: ยกเลิก sell order แล้ว market sell
                if order.get('sell_order_id'):
...
def predict_and_trade(current_ts):
    ...
    if prob >= CONFIDENCE_THRESHOLD:
        try:
            limit_buy_price = round(last_price * (1 - MAKER_BUY_OFFSET_PCT), PRICE_PRECISION)
            raw_qty = CAPITAL_PER_TRADE / last_price
            qty = float(int(raw_qty / QTY_STEP) * QTY_STEP)  # ปัดลงตาม step size
            qty = round(qty, QTY_PRECISION)
            tp = round(limit_buy_price * (1 + PROFIT_TARGET_PCT), PRICE_PRECISION)
            sl = round(limit_buy_price * (1 - STOP_LOSS_PCT), PRICE_PRECISION)
