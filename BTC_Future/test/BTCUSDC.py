            ...
            if res.get("result"):
                for update in res["result"]:
                    ...
                    if "message" in update and "text" in update["message"]:
                        ...
                        # แทรกคำสั่ง /holding
                        elif cmd == "/holding" and len(args) > 1:
                            try:
                                HOLDING_TIME = int(args[1])
                                send_tg_msg(f"⏳ Holding Time set to: {HOLDING_TIME} seconds")
...
def check_orders(current_price, current_ts):
    ...
    for order in active_orders[:]:
        ...
        if is_exit:
            ...
            # แทรกการดึงเวลา HH:mm:ss สำหรับ Log การขาย
            now_sold = datetime.datetime.now().strftime("%H:%M:%S")
            print(f"\n{color}[{now_sold}][{reason}] Sold: {current_price:.2f} | Net: {profit:.4f} FDUSD{C_RESET}")
...
def predict(data_list, last_price, current_ts):
    ...
    # แทรกการดึงเวลาสำหรับ Log การทำนายราคา (Predict)
    now_pred = datetime.datetime.now().strftime("%H:%M:%S")
...
# START
# ==========================================
start_full = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
